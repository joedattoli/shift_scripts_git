...
avgs_df = pd.DataFrame(columns = ['batter', 'bat_hand','bip', 'wOBA_avg','xwOBA_avg','standard_wOBA_avg','standard_xwOBA_avg','strat_wOBA_avg',
...
for k in range(len(player_info.index+1)):
    ...
    avgs_df = avgs_df.append(player_temp_df, ignore_index = True)
avgs_df = avgs_df.dropna()

## writes the dataframe above to a an excel file

with pd.ExcelWriter(r"C:\Users\joedattoli\Desktop\ShiftScripts\batter_means_file.xlsx") as writer:
    avgs_df.to_excel(writer)
#write raw balls to excel for later use
with pd.ExcelWriter(r"C:\Users\joedattoli\Desktop\ShiftScripts\raw_batted_balls.xlsx") as writer:
    batted_balls.to_excel(writer)

#auto_run next script so I can leave for the day and not be behind
runpy.run_path(r'C:\Users\joedattoli\Desktop\ShiftScripts\by_player _la10.py')