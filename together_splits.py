...
def weighted_mean_woba(dataframe):
    ...
    print(len(count_stand),len(count_strat),len(count_shift),len(together))
...
#normalizes the data
data_frame_normed = norm_plus(data_frame)

#makes a dataframe of the differences between actual wOBA and xwOBA based on EV and LA
dif_frame = difference_maker(data_frame)

print('line 173 1st')
##puts the normed frame and difference frame into two excel files

with pd.ExcelWriter(r"C:\Users\joedattoli\Desktop\ShiftScripts\difference_woba.xlsx") as writer:
    dif_frame.to_excel(writer)
with pd.ExcelWriter(r"C:\Users\joedattoli\Desktop\ShiftScripts\mean_data_plus.xlsx") as writer:
    data_frame_normed.to_excel(writer)

print('line 181 1st')    
#auto_run next script so I can leave for the day and not be behind
runpy.run_path(r'C:\Users\joedattoli\Desktop\ShiftScripts\together_splits_la_10.py')